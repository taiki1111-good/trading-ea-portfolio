...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Analyze HTF v2 diagnostic trade decomposition from existing logs.")
...
def load_and_join(decision_logs: Path, trade_logs: Path) -> tuple[pd.DataFrame, int]:
    ...
    _require_columns(ddf, ["timestamp", *HTF_COLUMNS], "decision_logs")
    ...
    # Keep latest row if timestamps duplicate.
    ddf = ddf.drop_duplicates(subset=["timestamp_utc"], keep="last")

    joined = tdf.merge(
        ddf[["timestamp_utc", *HTF_COLUMNS]],
        left_on="entry_time_utc",
        right_on="timestamp_utc",
        how="left",
    )
    unmatched_count = int(joined[HTF_COLUMNS[0]].isna().sum())
...
def write_markdown(path: Path, summary_df: pd.DataFrame, unmatched_count: int) -> None:
    lines = [
        "# HTF v2 Diagnostic Trade Group Summary",
        "",
        "- これは既存ログの後処理診断であり、収益性確認ではない。",
        "- HTF v2はentryを止めていない（diagnostic_only）。",
        "- 実filter化判断ではない。",
        "",
    ]
...
def run_analysis(decision_logs: Path, trade_logs: Path, output_dir: Path) -> tuple[Path, Path, Path, int]:
    ...
    analysis_out = output_dir / "htf_v2_trade_analysis.csv"
    out_cols = ["trade_id", "entry_time", "direction", "pnl", *HTF_COLUMNS]
    analysis_df[out_cols].to_csv(analysis_out, index=False, encoding="utf-8")

    group_df = build_group_summary(analysis_df)
    group_out = output_dir / "htf_v2_group_summary.csv"
    group_df.to_csv(group_out, index=False, encoding="utf-8")

    md_out = output_dir / "htf_v2_group_summary.md"
