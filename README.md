...
## 概要
モジュール分割、段階的テスト、判断理由の記録を重視して、自動売買EAの構成を設計・検証する個人プロジェクトです。
本リポジトリは、設計・検証・説明可能性を重視した研究/検証用EAフレームワークです。実際の取引システムとは接続しておらず、注文送信機能も実装していません。

## このプロジェクトで示すこと
- 売買ロジック単体ではなく、責務分離、ログ設計、検証手順を通じて、判断過程を後から追跡できる設計
- `Data` から `Evaluator` までの一方向フロー
- dry-run（実注文を行わない検証実行）、diagnostic comparison（採用前の診断比較）、shadow comparison（本体挙動に影響させない比較）を使い、実装済み・検証済み・未実装を分けて確認する進め方
- no-real-order integrity（実注文が発生していないことの整合確認）を含むログ検証

## まず見るべき文書
- 概要: `README.md`
- 外部説明: `docs/portfolio/portfolio_overview.md`
- アーキテクチャ: `docs/portfolio/architecture_for_portfolio.md`
- 面接向け説明: `docs/portfolio/interview_pitch.md`
...
## 現在の段階
主要モジュール（Data -> HTFContext -> LTFStructure -> Signal -> RiskFilter -> Execution -> Logger -> Evaluator）を一周実装し、判断過程の説明可能性を重視した分析・検証基盤として整備を進めています。

### 実装済み・到達点
- `Data` から `Evaluator` までの一方向フローを実装
- `tests/integration/test_end_to_end_minimal_pipeline.py` による E2E 最小統合テストを完了
- `Execution` は dry-run（実注文を行わない検証実行）用の skeleton として設計されており、実際の取引システムとの接続や注文送信機能は未実装
...
- `Lot Sizing shadow comparison`（本体のロット決定を変えずに固定lotとrisk-based lotを比較する仕組み）を採用済み
- CSV replay pipeline dry-run の representative run を完了し、no-real-order integrity（実注文が発生していないことの整合確認）を確認

### 注意・未実装
- 上記は設計・検証基盤としての到達点であり、収益性の確認や実運用の準備完了を意味しない。
- lot sizing 本体接続（risk-based lot の本線採用）は未実装。
- `PositionSizer` は placeholder であり、lot sizing 本体ではない。
- HTF は diagnostic comparison v0（採用前の診断比較）であり、本体filter採用ではない。
- `Lot Sizing shadow comparison` は shadow comparison（本体挙動に影響させない比較）であり、本体接続ではない。
...
- worklog内の個別runや数値は、収益性確認や運用準備完了を意味しません。
